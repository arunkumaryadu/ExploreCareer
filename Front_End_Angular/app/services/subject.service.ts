...
console.log(sub);
  return this.http.post<any>(this.endpoint + 'subject/add', JSON.stringify(sub), this.httpOptions).pipe(
    tap((sub) => console.log(`added Subject w/ id=${sub.name}`)),
    catchError(this.handleError<any>('addSubject'))
  );
}
getSubjects(): Observable<any> {
  return this.http.get(this.endpoint + 'subject/getAllSubject').pipe(
